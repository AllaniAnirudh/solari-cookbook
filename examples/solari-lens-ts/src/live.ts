...
export async function runLive(store: LensStore): Promise<string> {
  ...
  try {
    ...
    browserClient = new Solari({ apiKey: required("SOLARI_API_KEY") })
    ...
    await runBrowserAgent(run, model, page)
    ...
    await run.executeTool({ environment: "desktop", tool: "open", input: { executable }, execute: () => desktop.open(executable, [`${previewUrl}/payment`]) })
    await new Promise((resolve) => setTimeout(resolve, 2500))
    await runDesktopConfirmation(run, model, desktop)
...
async function runBrowserAgent(run: LensRun, model: OpenCodeModel, page: BrowserPage): Promise<void> {
  const messages: any[] = [{ role: "system", content: "You are investigating a synthetic checkout. Use the tools to observe the page and identify the blocker. Do not invent a cause. Stop after you have evidence." }, { role: "user", content: "Complete checkout for the test product, or identify and document the blocker." }]
  for (let step = 0; step < 12; step++) {
    const response = await model.complete(messages, browserTools, run.runId)
    ...
    for (const call of response.tool_calls) {
      ...
      if (call.function.name === "observe") {
        ...
        result = await run.executeTool({ environment: "browser", tool: "click", input: args, execute: () => page.getByRole(String(args.role), { name: String(args.name) }).click() })
      } else if (call.function.name === "type") {
        result = await run.executeTool({ environment: "browser", tool: "type", input: { label: args.label }, execute: () => page.getByLabel(String(args.label)).fill(String(args.value)) })
      } else {
        result = { error: `Unsupported tool ${call.function.name}` }
      }
      messages.push({ role: "tool", tool_call_id: call.id, content: typeof result === "object" ? JSON.stringify(result) : String(result) })
    }
  }
  run.decision({ summary: "Browser tool budget reached before a final assessment.", observation: "The model did not finish within 12 tool calls.", nextAction: "desktop confirmation" })
}

async function runDesktopConfirmation(run: LensRun, model: OpenCodeModel, desktop: Desktop): Promise<void> {
  const messages: any[] = [{ role: "system", content: "You are independently verifying a visible checkout state on a desktop. Use screenshots and choose only actions supported by the tools. Do not assume the browser agent's diagnosis." }, { role: "user", content: "Inspect the checkout screen and confirm whether the payment control can proceed. Use at most a few actions, then report what is visibly true." }]
  for (let step = 0; step < 10; step++) {
    const response = await model.complete(messages, desktopTools, run.runId)
    messages.push({ role: "assistant", content: response.content ?? null, tool_calls: response.tool_calls })
    if (!response.tool_calls?.length) {
